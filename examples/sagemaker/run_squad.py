...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    # Compute predictions
    output_prediction_file = os.path.join(args.model_dir, "predictions_{}.json".format(prefix))
    output_nbest_file = os.path.join(args.model_dir, "nbest_predictions_{}.json".format(prefix))

    if args.version_2_with_negative:
        output_null_log_odds_file = os.path.join(args.model_dir, "null_odds_{}.json".format(prefix))
    else:
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        if args.do_train:
            ...
            if args.eval_all_checkpoints:
                checkpoints = list(
                    os.path.dirname(c)
                    for c in sorted(glob.glob(args.model_dir + "/**/" + WEIGHTS_NAME, recursive=True))
                )
